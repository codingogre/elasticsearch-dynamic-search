...
class WeightCombiner {
  constructor(options = {}) {
    this.options = {
      ...
      // Bias configurations
      knowledgeSearchBias: 0.15,   // Base semantic bias for knowledge searches
      ...
      properNounLexicalWeight: 0.9,  // Lexical weight for proper noun queries
      properNounSemanticWeight: 0.1, // Semantic weight for proper noun queries
  ...
  combineWeights(queryAnalysis, contextualWeights, queryEnhancement, query) {
    ...
    // Apply knowledge search bias
    finalLexicalWeight = Math.max(this.options.minWeight, finalLexicalWeight - this.options.knowledgeSearchBias);
    finalSemanticWeight = Math.min(this.options.maxWeight, finalSemanticWeight + this.options.knowledgeSearchBias);
    reasoning.push(`Knowledge search bias - favoring semantic by ${Math.round(this.options.knowledgeSearchBias * 100)}%`);
    ...
    if (queryStats.wordCount >= 4) {
      ...
      reasoning.push(`Long query (${queryStats.wordCount} words) - boosted semantic weight by ${Math.round(semanticBoost * 100)}%`);
    } else if (queryStats.wordCount >= 2) {
      semanticBoost = this.options.mediumQueryBoost;
      reasoning.push(`Medium query (${queryStats.wordCount} words) - boosted semantic weight by ${Math.round(semanticBoost * 100)}%`);
    }
    ...
    if (queryEnhancement.detectedRegion) {
      ...
      reasoning.push(`Regional query detected (${queryEnhancement.detectedRegion}) - increased semantic understanding by ${Math.round(this.options.regionalBias * 100)}%`);
    }
  ...
  _handleProperNounQuery(queryAnalysis, contextualWeights, queryEnhancement, reasoning) {
    ...
    reasoning.push(`Short query (${wordCount} words) with proper nouns detected (${properNouns}) - favoring lexical search for exact matching`);
